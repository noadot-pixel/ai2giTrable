...
| `auth/signup.jsp` | 회원가입 **목업**. 입력 검사 후 "가입 완료" alert만 띄우고 로그인 화면으로 이동 (Firebase 계정 생성/DB 기록 없음). 새 계정이 필요하면 Firebase 콘솔 Authentication에서 직접 추가 |
| `mypage.jsp` | **보기 전용** 마이페이지. 프로필 카드 + 내가 쓴 글 목록(수정/삭제 가능) + 스크랩한 글 목록(스크랩 해제 가능) |
| `mypage/edit.jsp` | 프로필 수정 + 프로필 이미지 업로드(Storage) |
| `admin/index.jsp` | 관리자 메인. "회원 관리" 카드는 클릭 시 "준비 중" alert만 뜸(화면 삭제됨), "예시 게시글 시드 추가" 버튼 |
